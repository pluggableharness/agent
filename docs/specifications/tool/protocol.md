...
`Describe` reports this plugin build's own identity: request is empty (`DescribeRequest {}`), response is a `DescribeResponse` wrapping a single `pluggableharness.common.v1.ProducerRef producer`. MUST be implemented — every one of the six category protocols gains this RPC in this same protocol revision.
