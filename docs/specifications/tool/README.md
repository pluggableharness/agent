...
This category depends directly on [`model/`](../model/README.md): the common JSON-Schema subset tool authors write `input_schema`/`output_schema` within is the same one [`model/data-types.md#tool-schema`](../model/data-types.md#tool-schema) defines for model tool-calling declarations (one wire type, `pluggableharness.agent.schema.v1.Schema`, shared by both categories), and `Invoke`'s server-streaming-plus-cancellation shape reuses [`model/README.md`](../model/README.md#transport--lifecycle)'s `StreamCompletion` pattern verbatim. See [`architecture.md`](../architecture.md) for the surrounding system (plan/apply gate, hook dispatch, Emit→Render→Paint, state backend) — this directory only covers the tool-provider RPC surface, data types, and reference catalog in detail.
