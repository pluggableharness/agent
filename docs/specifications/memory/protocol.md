...
- A memory provider is implicitly subscribed to `post-response` (`observe` mode, fires every turn) and `session-end` (fires once, unconditionally) — giving every provider a guaranteed last chance to persist something even if its own turn-by-turn heuristic never triggered mid-session. Nothing in this protocol prescribes *when* within that stream a provider decides to call its own internal write logic.
