...
`model_target` is the same `ModelTarget` shape [`memory/data-types.md`](../memory/data-types.md)'s `RecallRequest` carries — a rich "what am I assembling for" descriptor derived from the resolved model provider's `ModelSpec` ([`provider/data-types.md#modelspec`](../provider/data-types.md#modelspec)), distinct from the narrower `ModelRef` that [`kernel-callbacks.md#counttokens`](../kernel-callbacks.md#counttokens) uses to select a tokenizer. It lets a provider tailor content — and compute tokens against the right budget — for the model that will actually consume it, not just the one configured at session start; a sub-agent routed to a smaller model gets a correspondingly smaller `effective_ceiling` here automatically (see [`architecture.md#context-budget`](../architecture.md#context-budget)).
