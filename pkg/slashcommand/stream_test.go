...
// slashcommandv1.SlashCommandService_InvokeServer
// (grpc.ServerStreamingServer[InvokeResponse]), per
// .claude/rules/go-testing.md's "fakes, not mocking frameworks" rule.
type fakeInvokeServerStream struct {
	ctx     context.Context
	sendErr error

	mu   sync.Mutex
	sent []*slashcommandv1.InvokeResponse
}
...
func (f *fakeInvokeServerStream) Send(r *slashcommandv1.InvokeResponse) error {
	if f.sendErr != nil {
		return f.sendErr
	}
	f.mu.Lock()
...
func (f *fakeInvokeServerStream) events() []*slashcommandv1.InvokeResponse {
	f.mu.Lock()
	defer f.mu.Unlock()
	return append([]*slashcommandv1.InvokeResponse(nil), f.sent...)
}
...
func (f *fakeInvokeServerStream) SendMsg(m any) error {
	return f.Send(m.(*slashcommandv1.InvokeResponse))
}
func (f *fakeInvokeServerStream) RecvMsg(any) error { return io.EOF }

var _ slashcommandv1.SlashCommandService_InvokeServer = (*fakeInvokeServerStream)(nil)
...
func TestStreamSendTerminalClosesStream(t *testing.T) {
	...
	if err := s.Send(NewOutputChunkEvent(tool.OutputStreamStdout, []byte("a"))); err != nil {
	...
	err := s.Send(NewErrorEvent(&tool.Error{Category: tool.ErrorCategoryUnknown, Message: "too late"}))
...
func TestStreamResultAfterCancelRejected(t *testing.T) {
	...
	if err := s.Send(NewErrorEvent(&tool.Error{Category: tool.ErrorCategoryCancelled, Message: "cancelled"})); err != nil {
...
func TestStreamSendPropagatesTransportError(t *testing.T) {
	...
	err := s.Send(NewOutputChunkEvent(tool.OutputStreamStdout, []byte("x")))
