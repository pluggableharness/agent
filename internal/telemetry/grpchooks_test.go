...
func TestResourceEnv(t *testing.T) {
	...
	want := "pluggableharness.producer.category=tool,pluggableharness.producer.name=ripgrep,pluggableharness.producer.version=1.2.3"
	if got != want {
...
func TestResourceEnv_escapesSpecialCharacters(t *testing.T) {
	...
	want := "pluggableharness.producer.category=tool,pluggableharness.producer.name=my+tool,pluggableharness.producer.version=1.0.0%2Cbeta"
	if got != want {
		t.Errorf("ResourceEnv = %q, want %q", got, want)
	}
}
